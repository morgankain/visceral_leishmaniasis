...
parameters {
  ...
  real temp_lambda_bar_sq;

  // Slopes for probability piece
  real gdp_theta_bar;
...
for (j in 1:N) {
  ...
  inv_theta[j] = alpha_theta_bar + gdp_theta_bar * gdp[j];
...
for (jj in 1:N_out) {
  ...
  inv_theta_out[jj] = alpha_theta_bar + gdp_theta_bar * gdp_out[jj];
