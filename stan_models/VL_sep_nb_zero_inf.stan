...
parameters {
  ...
  real temp_theta_bar;                  // slope coefficients for probability piece
  real precip_theta_bar;       
  real year_theta_bar;  
...
transformed parameters {
  ...
  real alpha_theta[N_loc];              // location-specific intercepts for probability
...
for (j in 1:N) {
  ...
  theta[j]      = inv_logit(alpha_theta[loc_id[j]] + 
    temp_theta_bar * temp[j] +
    precip_theta_bar * precip[j] +
    year_theta_bar * year[j]);
...
// fixed effects
   ...
   temp_theta_bar ~ normal(0, 3);
   year_theta_bar ~ normal(0, 3);
