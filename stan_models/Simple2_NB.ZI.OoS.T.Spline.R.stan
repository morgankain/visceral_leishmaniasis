...
parameters {
  ...
  // Ghitza & Gelman used normal(delta[i-1],1) for sampling deltas, but that can lead to overfitting, so use a hyperprior
  real<lower=0, upper=10> delta_sigma;  
  
  // Year_deltas shorter than max_years, because the first delta logically has to be 0
  vector[max_year-1] year_deltas_bar;   
...
transformed parameters {
  ...
  matrix[N_loc, max_year] year_mu;
    
  // real_deltas is just year_deltas with 0 concatenated to the front
  vector[max_year] real_deltas_bar;
  matrix[N_loc, max_year] real_deltas_eps;
